...
const YANDEX_STATE_KEY = 'yandex_auth_state';

export function saveYandexState(state: string) {
  sessionStorage.setItem(YANDEX_STATE_KEY, state);
}

export function loadYandexState(): string | null {
  return sessionStorage.getItem(YANDEX_STATE_KEY);
}
...
export default function YandexCallbackPage() {
  ...
  useEffect(() => {
    ...
    if (!code || !state) {
      setStatus('error');
      setErrorMsg('Нет параметров от Яндекса. Попробуй ещё раз.');
      return;
    }

    const isWebFlow = loadYandexState() === state;

    api.yandexExchange(code, state)
      .then((r) => {
        ...
        if (isWebFlow) {
          ...
          // Mobile flow: browser was opened from the app — just show success
          setStatus('done');
        }
      })
      .catch((e) => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-ink">
        <div className="text-center px-6">
          ...
          <p className="text-cream text-lg font-serif">Авторизация прошла успешно</p>
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-ink">
        <div className="text-center px-6">
          ...
          <button
            onClick={() => navigate('/')}
            className="mt-4 text-stone text-sm underline"
          >
            На главную
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-ink">
      <div className="text-center px-6">
        ...
        <p className="text-stone text-sm">Входим через Яндекс…</p>
      </div>
    </div>
  );
}
