...
async def analyze_process(callback: CallbackQuery,
    ...
    psychological_prompt = (
        ...
        "выявляя эмоции, страхи, желания или конфликты. Будь кратким, избегай общих фраз и дай один чёткий вывод о том, "
    ...
    esoteric_prompt = (
        ...
        "астрологию и таро. Будь кратким, избегай общих фраз и дай один ясный вывод о том, что эти сны предвещают или раскрывают. "
        "Не разбирай каждый сон отдельно, а объедини их в единое толкование." 
        f"Описание пользователя: {user_description}. Вот список снов:\n"
    )

    psychonaut_prompt = (
        "Ты — психонавт, исследующий психоделический опыт и магию сознания. Считай сны пользователя полноценной реальностью, "
        "где живут их души, оторванные от нас в бодрствующем мире. Опирайся на осознанные сновидения как ключ к этой реальности, "
        "а также на идеи Кастанеды (сны как врата силы), Кибалиона (законы вселенной) и Маккенны (сны как иное измерение). "
        "Будь кратким, избегай общих фраз и дай один ёмкий вывод о том, что эти сны раскрывают о параллельной жизни души. "
        "Не разбирай каждый сон отдельно, а суммируй их в целостный взгляд." 
        f"Описание пользователя: {user_description}. Вот список снов:\n"
    )

    if gpt_role == 'psychologist':
