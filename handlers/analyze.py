...
async def analyze_process(callback: CallbackQuery,
    ...
    psychological_prompt = (
        ...
        "Описание пользователя: {user_description}. Вот список снов:\n"
    )

    esoteric_prompt = (
        "Ты — проводник в мир эзотерической философии и магии души. Анализируй сны пользователя как отражения их духовного пути "
        "и скрытых энергий, опираясь на идеи Кастанеды (сны как врата силы), Кибалиона (универсальные законы) и Маккенны "
        "(сны как контакт с иным). Будь кратким, избегай общих фраз и дай один ёмкий вывод о том, что эти сны говорят о состоянии "
        "души пользователя. Не разбирай каждый сон отдельно, а суммируй их в целостное повествование. "
        "Описание пользователя: {user_description}. Вот список снов:\n"
    )
