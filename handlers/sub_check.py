...
@start.message(CommandStart(deep_link_encoded=True))
...
@start.callback_query(SubCheckSG.sub_check, F.data[:16] == "check_subscribe_")
async def check_subscribe(callback: CallbackQuery,
    ...
    user_channel_status = await bot.get_chat_member(chat_id=str(channel_id.id), 
                                                    user_id=user_id)
